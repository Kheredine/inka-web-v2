...
type Step = 'enter-name' | 'show-code' | 'sign-in'
...
export default function RegisterPage() {
  ...
  const handleContinue = () => {
    router.push(`/login?username=${encodeURIComponent(username.trim())}&code=${generatedPin}`)
  }
    ...
    <main style={{ minHeight: '100dvh', background: colors.background, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: spacing.xl }}>
      <div style={{ width: '100%', maxWidth: 360 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: spacing.xxl }}>
          ...
          <p style={{ color: colors.textMuted, fontSize: typography.sm.fontSize, marginTop: spacing.xs }}>Créer mon compte</p>
        </div>

        {step === 'enter-name' && (
          <form onSubmit={handleGenerateCode} style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
            <div>
            ...
            <Button label="Générer mon code" loading={loading} type="submit" disabled={!username.trim()} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: colors.textMuted, fontSize: typography.xs.fontSize, marginBottom: spacing.sm, letterSpacing: 0.5, textTransform: 'uppercase' }}>
                Compte créé pour <strong style={{ color: colors.textPrimary }}>{username}</strong>
              </p>
